...
public:
    ...
    ros::Publisher pubSensorCloudRegisteredRaw;
    ...
    pcl::PointCloud<PointType>::Ptr   sensorRawRegCloud; // register the raw sensor cloud on odom
    ...
    mapOptimization()
    {
        ...
        pubSensorCloudRegisteredRaw = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/sensor_cloud_registered_raw", 1);
    ...
    void publishGlobalMap()
    {
        ...
        publishCloud(&pubLaserCloudSurround, globalMapKeyFramesDS, timeLaserInfoStamp, "odom_lio");    
    }
    ...
    void performLoopClosure()
    {
        ...
        *nearHistoryKeyFrameCloud = *cloud_temp;
        ...
        publishCloud(&pubHistoryKeyFrames, nearHistoryKeyFrameCloud, timeLaserInfoStamp, "odom_lio");
        ...
        if (pubIcpKeyFrames.getNumSubscribers() != 0){
            ...
            publishCloud(&pubIcpKeyFrames, closed_cloud, timeLaserInfoStamp, "odom_lio");
        }
    ...
    void addOdomFactor()
    {
        if (cloudKeyPoses3D->points.empty())
        {
            noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-2, 1e-2, M_PI*M_PI/25, 1e8, 1e8, 1e8).finished()); // rad*rad, meter*meter
            gtSAMgraph.add(PriorFactor<Pose3>(0, trans2gtsamPose(transformTobeMapped), priorNoise));
            initialEstimate.insert(0, trans2gtsamPose(transformTobeMapped));
        }else{
            noiseModel::Diagonal::shared_ptr odometryNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-3, 1e-3, 1e-3, 1e-2, 1e-2, 1e-2).finished());
    ...
    void publishOdometry()
    {
        ...
        laserOdometryROS.header.frame_id = "odom_lio";
    ...
    void publishFrames()
    {
        if (cloudKeyPoses3D->points.empty())
            return;
        // publish key poses
        publishCloud(&pubKeyPoses, cloudKeyPoses3D, timeLaserInfoStamp, "odom_lio");
        // Publish surrounding key frames
        publishCloud(&pubRecentKeyFrames, laserCloudSurfFromMapDS, timeLaserInfoStamp, "odom_lio");
        ...
        if (pubRecentKeyFrame.getNumSubscribers() != 0)
        {
            ...
            *cloudOut += *transformPointCloud(laserCloudSurfLastDS,    &thisPose6D);
            publishCloud(&pubRecentKeyFrame, cloudOut, timeLaserInfoStamp, "odom_lio");
        }
        ...
        if (pubCloudRegisteredRaw.getNumSubscribers() != 0)
        {
            ...
            *cloudOut = *transformPointCloud(cloudOut,  &thisPose6D);
            publishCloud(&pubCloudRegisteredRaw, cloudOut, timeLaserInfoStamp, "odom_lio");


        }
        if (pubSensorCloudRegisteredRaw.getNumSubscribers() != 0)
        {
            pcl::PointCloud<PointType>::Ptr rawCloud(new pcl::PointCloud<PointType>());
            pcl::fromROSMsg(cloudInfo.raw_sensor_cloud, *rawCloud);
            PointTypePose thisPose6D = trans2PointTypePose(transformTobeMapped);
            *rawCloud = *transformPointCloud(rawCloud,  &thisPose6D);
            publishCloud(&pubSensorCloudRegisteredRaw, rawCloud, timeLaserInfoStamp, "odom_lio");
        }
        ...
        if (pubPath.getNumSubscribers() != 0)
        {
            ...
            globalPath.header.frame_id = "odom_lio";
